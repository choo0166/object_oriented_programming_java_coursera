...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		// Set Marker size
		float markerSize = CityMarker.TRI_SIZE; // Length of sides of triangle
		...
		fill(color(139, 0, 0));
		triangle(50, 100, 
				50 + markerSize/2, 100 - markerSize, 
				50 + markerSize, 100);
		fill(color(255, 255, 255));
		ellipse(50 + markerSize/2, 125, markerSize, markerSize);
		rect(50, 150, markerSize, markerSize);
	
		fill(0, 0, 0);
		text("City Marker", 75, 95);
		text("Land Quake", 75, 121);
		text("Ocean Quake", 75, 152);
		text("Size ~ Magnitude", 50, 175);
		
		fill(255, 255, 0);
		ellipse(50 + markerSize/2, 200, markerSize, markerSize);
		fill(0, 0, 255);
		ellipse(50 + markerSize/2, 225, markerSize, markerSize);
		fill(255, 0, 0);
		ellipse(50 + markerSize/2, 250, markerSize, markerSize);
		
		fill(0, 0, 0);
		text("Shallow", 75, 197);
		text("Intermediate", 75, 222);
		text("Deep", 75, 250);
		
		fill(255, 255, 255);
		ellipse(50 + markerSize/2, 275, markerSize, markerSize);
		line(50, 275 + markerSize/2, 50 + markerSize, 275 - markerSize/2);
		line(50, 275 - markerSize/2, 50 + markerSize, 275 + markerSize/2);
	...
	private void printQuakes() 
	{
		...
		// Initialize dictionary for storing LandQuake counts
		HashMap<String, Integer> LandQuakes = new HashMap<String, Integer>();
		int numOceanQuakes = 0;
		
		for (Marker cm: countryMarkers) {
			String name = (String)cm.getProperty("name");
			LandQuakes.put(name, 0);
		}
		
		for (Marker m: quakeMarkers) {
			if (m instanceof LandQuakeMarker) {
				String country = (String)m.getProperty("country");
				// Add country as key if it does not exist and 1 as value
				// Otherwise sum 1 to the value linked to key
				LandQuakes.merge(country, 1, Integer::sum);
			}
			else { numOceanQuakes++; }
		}
		
		for (Entry<String, Integer> entry: LandQuakes.entrySet()) {
