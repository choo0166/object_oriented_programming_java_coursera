...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getNeighbors(int v) {
		...
		for (int j = 0; j < getNumVertices(); j++) {
			for (int k = 0; k < adjMatrix[v][j]; k++) { // adjMat[i][j] > 1 means more than 1 edge from node i -> j
				neighbors.add(j);
			}
		}
		return neighbors;
	...
	public List<Integer> getInNeighbors(int v) {
		...
		for (int i = 0; i < getNumVertices(); i ++) {
			for (int k = 0; k < adjMatrix[i][v]; k++) { // More than 1 edge going from node i -> v
				inNeighbors.add(i);
			}
		}
		return inNeighbors;
	...
	public List<Integer> getDistance2(int v) {
		...
		for (int x = 0; x < getNumVertices(); x++) {
			if (adjMatrix[v][x] >= 1) { // 1st hop
				for (int y = 0; y < getNumVertices(); y++) { // 2nd hop
					for (int k = 0; k <adjMatrix[x][y]; k++) { ans.add(y); }
				}
			}
		}
		return ans;
